...
from LMSCNet.common.mmmetrics import Metrics
...
def train(model, optimizer, scheduler, dataset, _cfg, start_epoch, logger, tbwriter):
  ...
  for epoch in range(start_epoch, nbr_epochs+1):
    ...
    '''for scale in metrics.evaluator.keys():
      tbwriter.add_scalar('train_performance/{}/IoU'.format(scale), np.array(metrics.get_occupancy_IoU(scale)), epoch-1)
    ...
    for scale in metrics.evaluator.keys():
      out_str = ' / '.join([str(elem) for elem in metrics.get_occupancy_IoU(scale)])
      loss_scale = metrics.losses_track.train_losses['semantic_{}'.format(scale)].item()/metrics.losses_track.train_iteration_counts
      logger.info('=> [Epoch {} - Scale {}: Loss = {:.6f} - IoU = {} ]'
                  .format(epoch, scale, loss_scale, out_str))
...
def validate(model, dset, _cfg, epoch, logger, tbwriter, metrics):
  ...
  with torch.no_grad():
    ...
    metrics.update_best_metric_record(0, IoU_1_1, epoch_loss.item(), epoch)
...
def main():
  ...
  logger.info('=> [Best performance: Epoch {} - IoU {}]'.format(best_record['epoch'], best_record['IoU']))
