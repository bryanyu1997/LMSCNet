...
class ResNet18_cascade(nn.Module):

  def __init__(self, class_num, input_dimensions, class_frequencies, downsample=False, mul=1, part=4):
    '''
    SSCNet architecture
    ...
    if not self.downsample:
      ...
      self.class_head = nn.Sequential(
        nn.Conv2d(512//mul, 512//mul, 3, padding=1, bias=False),
        nn.BatchNorm2d(512//mul),
        nn.ReLU(inplace=True),
        nn.Conv2d(512//mul, class_num*32, 1),
    )
    else:
      self.stem = nn.Sequential(
          nn.Conv2d(64, 64, 3, padding=1, bias=False),
      ...
      self.class_head = nn.Sequential(
        ...
        nn.BatchNorm2d(512//mul),
        nn.ReLU(inplace=True),
        nn.Conv2d(512//mul, class_num*16, 1),
    )
    self.decoder = Decoder(mul)
    self.cascade = Cascade(mul, part, 16)
  ...
  def forward(self, x):
    ...
    if not self.downsample:
      ...
      output = self.cascade(output)
      output = F.interpolate(output, scale_factor=2, mode='trilinear')

    # Take back to [W, H, D] axis order
    out_scale_1_1__3D = output.permute(0, 1, 3, 2, 4)  # [bs, C, D, H, W] -> [bs, C, H, D, W]
...
class Decoder(nn.Module):
    def __init__(self, mul):
        ...
        self.lateral4 = conv2dbnrelu(512, 512//mul)
        self.lateral3 = conv2dbnrelu(256, 256//mul)
        self.lateral2 = conv2dbnrelu(128, 128//mul)
        self.lateral1 = conv2dbnrelu(64, 64//mul)
        self.fuse34 = conv2dbnrelu(512//mul+256//mul, 512//mul)
        self.fuse23 = conv2dbnrelu(512//mul+128//mul, 512//mul)
        self.fuse12 = conv2dbnrelu(512//mul+64//mul, 512//mul)
...
class Cascade(nn.Module):
    def __init__(self, mul, part=4, z_dim=32):
        super(Cascade,self).__init__()
        self.layer = [make_cascade(512//mul+20*z_dim//part, 20*z_dim//part) 
                      if l!=0 else make_cascade(512//mul, 20*z_dim//part) for l in range(part)]
        self.z_dim = z_dim
    ...
    def forward(self, x):
        ...
        for layer in self.layer:
            layer = layer.to(inputs.device)
            res = layer(inputs)
            inputs = torch.cat([x, res.detach()], dim=1)
            output.append(res)

        output_scene = torch.cat(output,dim=1)
        output_scene = output_scene.reshape(x.shape[0],-1, self.z_dim, x.shape[-2], x.shape[-1])
        return output_scene