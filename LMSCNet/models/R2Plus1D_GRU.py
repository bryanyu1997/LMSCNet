...
class R2Plus1D_GRU(nn.Module):
  ...
  def __init__(self, class_num, input_dimensions, class_frequencies):
    ...
    self.hidden_dim = 32
    self.gru = nn.GRU(64,self.hidden_dim,1)
  ...
  def forward(self, x):
    ...
    output_2d, h = self.gru(reconst_2d)

    output = output_2d.reshape(reconst.shape[0],reconst.shape[1],reconst.shape[2],reconst.shape[3],self.hidden_dim).permute(1,4,0,2,3)
