...
class R2Plus1D_sampling(nn.Module):

  def __init__(self, class_num, input_dimensions, class_frequencies, sampling=0):
    '''
    SSCNet architecture
  ...
  def compute_loss(self, scores, data):
    ...
    loss = criterion(scores['pred_semantic_1_1'], data['3D_LABEL']['1_1'].long())

    # Sampling with Free-Space
    mask_obj = target[(target!=0)&(target!=255)]
    K = len(mask_obj) * self.sampling_para
    loss_free = loss[target==0].topk(k=K)[0] 
    loss_obj = loss[(target!=0)&(target!=255)]
    loss_1_1 = torch.cat([loss_free, loss_obj]).mean()
