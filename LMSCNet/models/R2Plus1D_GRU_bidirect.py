...
class R2Plus1D_GRU_bidirect(nn.Module):

  def __init__(self, class_num, input_dimensions, class_frequencies, hidden_dim, gru_mode=False):
    '''
    SSCNet architecture
    ...
    if gru_mode:
      self.gru = nn.GRU(64,self.hidden_dim//2,1,bidirectional=gru_mode)
    else:
      self.gru = nn.GRU(64,self.hidden_dim,1,bidirectional=gru_mode)

      
    self.semantic = nn.Sequential(
            nn.Conv3d(in_channels=self.hidden_dim,out_channels=20,kernel_size=1,stride=1,padding=0),
            nn.BatchNorm3d(20),
            nn.ReLU(inplace=True),)
    self.residual = nn.Sequential(
            nn.Conv3d(in_channels=64,out_channels=self.hidden_dim,kernel_size=1,stride=1,padding=0),
  ...
  def forward(self, x):
    ...
    reconst = self.decoder(feature_lst)
    output_res = self.residual(reconst)
    # Implement GRU
    # (B,C,D,H,W) -> (D,B,H,W,C)
    reconst = reconst.permute(2,0,3,4,1)
    reconst_2d = reconst.reshape(reconst.shape[0],-1,reconst.shape[4])
    output_2d, h = torch.utils.checkpoint.checkpoint(self.gru, reconst_2d)
    output_2d = output_2d.reshape(reconst.shape[0],reconst.shape[1],reconst.shape[2],reconst.shape[3],self.hidden_dim).permute(1,4,0,2,3)

    output = output_res + output_2d
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        #self.layer_5 = nn.Conv3d(in_channels=64,out_channels=1,kernel_size=1,stride=1,padding=0)
