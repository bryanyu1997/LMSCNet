...
class VideoResNet_fast_tune(nn.Module):
  ...
  def __init__(self, class_num, input_dimensions, class_frequencies):
    ...
    ch_lst = [16,32,128,256]
    self.encoder = VideoResNet(num_channels=ch_lst, stem=FastStem, layers=[1,1,3,3])
...
class Decoder(nn.Module):
    def __init__(self, ch_lst, de_ch):
        ...
        self.lateral4 = nn.Sequential(
            Conv3d3x3BN(ch_lst[3], de_ch), nn.ReLU(inplace=True),
        ...
        self.layer5 = nn.Sequential(
            nn.Conv3d(de_ch, de_ch, 1, bias=False),
            nn.BatchNorm3d(de_ch),
            nn.ReLU(inplace=True),
            nn.Conv3d(in_channels=de_ch,out_channels=20,kernel_size=1,stride=1,padding=0),
        )
