...
from LMSCNet.common.model_kinetics import get_model
...
from LMSCNet.data.Kinetics400_crop import Kinetics400
...
def train(model, optimizer, scheduler, dataset, _cfg, start_epoch, logger, tbwriter):
  ...
  metrics = Metrics(400, nbr_iterations, model.get_scales())
  ...
  for epoch in range(start_epoch, nbr_epochs+1):
    ...
    for t, (data, indices) in enumerate(dset):
      ...
      acc.append(calculate_accuracy(scores['pred_semantic_1_1'], model.get_target(data)['1_1']))

      if (t + 1) % _cfg._dict['TRAIN']['SUMMARY_PERIOD'] == 0:
        loss_print = '=> Epoch [{}/{}], Iteration [{}/{}], Accuracy [{}], Learn Rate: {}, Train Losses: '\
          .format(epoch, nbr_epochs, t+1, len(dset), sum(acc)/len(acc), scheduler.get_lr()[0])
    ...
    for scale in metrics.evaluator.keys():
      ...
      logger.info('5=> [Epoch {} - Part {}: Loss = {:.6f} - Accuracy = {:.6f} - mIoU = {:.6f} - IoU = {:.6f} '
                  '- P = {:.6f} - R = {:.6f} - F1 = {:.6f}]'
                  .format(epoch, '1_1' ,loss_scale,
                          sum(acc)/len(acc),
    ...
    metrics.reset_evaluator()
    checkpoint_info = validate(model, dataset['valid'], _cfg, epoch, logger, tbwriter, metrics)
...
def validate(model, dset, _cfg, epoch, logger, tbwriter, metrics):
  ...
  #model.load_state_dict(torch.load('/media/NFS/bryan/LMSCNet/LMSCNet/SSC_out/R2Plus1D/chkpt/best-metric/weights_epoch_041.pth'))
  model.eval()
  ...
  with torch.no_grad():
    ...
    for t, (data, indices) in enumerate(dset):
      ...
      if (t + 1) % _cfg._dict['VAL']['SUMMARY_PERIOD'] == 0:
        loss_print = '=> Epoch [{}/{}], Iteration [{}/{}], Accuracy [{}], Train Losses: '.format(epoch, nbr_epochs, t+1, len(dset), sum(acc)/len(acc))
        for key in loss.keys(): loss_print += '{} = {:.6f},  '.format(key, loss[key])
        logger.info(loss_print[:-3])
    ...
    for scale in metrics.evaluator.keys():
      ...
      tbwriter.add_scalar('validation_performance/{}/IoU'.format(scale), sum(acc)/len(acc), epoch-1)
    ...
    for scale in metrics.evaluator.keys():
      loss_scale = metrics.losses_track.validation_losses['semantic_{}'.format(scale)].item()/1
      logger.info('=> [Epoch {} - Part {}: Loss = {:.6f} - Accuracy = {:.6f} - mIoU = {:.6f} - IoU = {:.6f} '
                  '- P = {:.6f} - R = {:.6f} - F1 = {:.6f}]'
                  .format(epoch,  '1_1', loss_scale,
                          sum(acc)/len(acc),
                          metrics.get_semantics_mIoU(scale).item(),
...
def main():
  ...
  train_set = Kinetics400(root=dataset_f, file_root=_cfg._dict['TRAIN']['CSV_NAME'], frames_per_clip=16, transform=RandomResizedCrop(
                size=[480, 360], scale=(1.0, 1.0), ratio=(1., 1.)))
  valid_set = Kinetics400(root=dataset_f, file_root=_cfg._dict['VAL']['CSV_NAME'], frames_per_clip=16, transform=RandomResizedCrop(
                size=[480, 360], scale=(1.0, 1.0), ratio=(1., 1.)))
  dataset = {}
  dataset['train'] = DataLoader(train_set, batch_size=_cfg._dict['TRAIN']['BATCH_SIZE'], num_workers=_cfg._dict['DATALOADER']['NUM_WORKERS'], shuffle=True, sampler=None)
  dataset['valid'] = DataLoader(valid_set, batch_size=_cfg._dict['VAL']['BATCH_SIZE'], num_workers=_cfg._dict['DATALOADER']['NUM_WORKERS'], shuffle=True, sampler=None)
